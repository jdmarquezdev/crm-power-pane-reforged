$(function () {
    console.log("CrmPowerPane: Main UI Script Initializing... Version: __VERSION__");
    var CrmPowerPane = {
        ApplicationType: {
                    ...
                    this.ShowData = function (callback) {
                        ...
                        for (var key in this.Parameters) {
                            ...
                            if (Array.isArray(p.value)) {
                                var li = "<li><span class='crm-power-pane-popup-input-text'>" + CrmPowerPane.Utils.EscapeHtml(p.label) + ":</span>";
                                p.value.forEach(function (item) {
                                    ...
                                    // Use EscapeHtml for name to prevent XSS
                                    li += "<li><a href='#' onclick='window.open(\"" + url + "\")'>" + CrmPowerPane.Utils.EscapeHtml(item.name) + "</a></li>"
                                });
                                li += "</li>";
        ...
        RegisterEvents: function () {
            ...
            $("#record-url").click(function () {
                try {
                    var header = CrmPowerPane.L.t("record_url");
                    var description = CrmPowerPane.L.t("record_url_desc");
                    ...
                    var result = [{
                        label: CrmPowerPane.L.t("record_url"),
                        value: url.join("")
                    }];
                    ...
                    if (Xrm.Utility && Xrm.Utility.getGlobalContext && Xrm.Utility.getGlobalContext().getCurrentAppProperties) {
                        Xrm.Utility.getGlobalContext().getCurrentAppProperties().then(function (appDetails) {
                            ...
                            result.push({
                                label: CrmPowerPane.L.t("record_url_app"),
                                value: url.join("")
                            });
            ...
            $("#user-info").click(function () {
                try {
                            ...
                            service.onreadystatechange = function () {
                                if (service.readyState === 4) {
                                    if (service.status === 200) {
                                        ...
                                        resolve(results.map(function (t) {
                                            return {
                                                name: t.Name,
                                                id: t.TeamId,
                                                entityType: "team"
                    ...
                    Promise.all([getUserRoles(), getUserTeams()])
                        ...
                        .catch(function (error) {
                            CrmPowerPane.UI.ShowNotification("An error occurred while getting the user information: " + error, "error");
                        });
                } catch (e) {
                            ...
                            if ($label) {
                                $label.attr("title", attributeLogicalName), $label.off("click").click(function () {
                                    if (navigator.clipboard && navigator.clipboard.writeText) {
                                        navigator.clipboard.writeText(attributeLogicalName).then(function () {
                                            CrmPowerPane.UI.ShowNotification("Copied <b>\"" + CrmPowerPane.Utils.EscapeHtml(attributeLogicalName) + "\"</b> to clipboard.", "success");
                                        }).catch(function (err) {
                                            console.error("Clipboard API failed", err);
                                            CrmPowerPane.UI.ShowNotification("Copy options unavailable.", "error");
                                        });
                                    } else {
                                        // Fallback legacy
    ...
    var lang = $(".crm-power-pane-container").attr("data-language") || "en";
    $("#crm-power-pane-language-selector").val(lang);

    $(document).on("change", "#crm-power-pane-language-selector", function () {
        var newLang = $(this).val();
        window.postMessage({
            type: "CRM_POWER_PANE_SAVE_LANGUAGE",
            language: newLang
        }, "*");

        // Reload pane with new language (simple Hack: close and reopen or just reload page prompt)
        CrmPowerPane.UI.ShowNotification("Language changed. Please reopen the pane or refresh to apply.", "info");
    });
